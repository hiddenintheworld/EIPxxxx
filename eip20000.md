...
## Simple Summary
This proposal introduces an additional transaction feature to Ethereum's current consensus mechanism. The feature allows transactions to expire after a certain period if they are not mined, enabling users to cancel transactions without having to replace them with a new one and pay a higher gas fee.
...
* **Lack of flexbility for user**: It's insane to not being able to cancel an uncessary transaction like when using Uniswap to swap tokens or buying a NFT
* **Network traffic with useless transaction meant to fail**: watch a transaction fail while it has already stuck for so long, can't cancel but know it will fail
